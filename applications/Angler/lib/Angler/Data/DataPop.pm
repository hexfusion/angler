...
sub pop_users{
    ...
    for(0...$no_users){
        my $user = Angler::Data::DataGen::users();
...
sub pop_attributes{
    ...
    # create size attribute
        my $size_data = {
            ...
            type             => 'variant',
            priority         => 1,
            attribute_values => Angler::Data::DataGen::size()
        };
        my $size_att = $shop_schema->resultset('Attribute')->create($size_data);
    
    foreach my $height ( @{Angler::Data::DataGen::height()} ) {
        $shop_schema->resultset('Attribute')
...
sub pop_products{
    ...
    my $so_far;
    my $skus = Angler::Data::DataGen::uniqe_varchar($no_products);
    my @products;
    foreach(@{$skus}){
        my $product = Angler::Data::DataGen::products($_);
        my $variants = Angler::Data::DataGen::variants($product, $no_colors);
        try {
...
sub pop_navigation{
    ...
    while (my $product = $products->next) {
        $count++;
        my $ran = Angler::Data::DataGen::rand_int(0, $#navs);
        my $nav = $navs[$ran];
...
sub pop_orders{
    ...
    foreach(@users){
        ...
        while($count < $no_orders){
            $count++;
            Angler::Data::DataGen::orders($_->id);
        }

# Interchange UserTag ebay_enditem v1.0
#
...
Usertag ebay_enditem Order itemid
Usertag ebay_enditem Routine <<EOR
...
sub {
    	...
    	my $content = "<?xml version='1.0' encoding='iso-8859-1'?><request><RequestToken>$request_token</RequestToken><ErrorLevel>1</ErrorLevel><DetailLevel>0</DetailLevel><Verb>EndItem</Verb><SiteId>0</SiteId><ItemId>$itemid</ItemId><EndCode>2</EndCode></request>";
	...
	if ( index($response, '<?xml') == 0 ) {
    		...
    		my $result = $eBay->{EndItem};
	       	$Scratch->{item_id_response} = $result->{ItemId};
	        $Scratch->{enditem_log} = $response;
	}
	else {

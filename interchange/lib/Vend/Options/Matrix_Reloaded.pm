# Vend::Options::Matrix_Reloaded - Interchange Matrix product options
#
...
package Vend::Options::Matrix_Reloaded;

$VERSION = substr(q$Revision: 1.17 $, 10);

=head1 NAME

Vend::Options::Matrix_Reloaded - Interchange Matrix Options Support
...
The Vend::Options::Matrix_Reloaded module implements matrix product options for
...
%Default = (
				no_pricing => 1,
				item_add_routine => 'Vend::Options::Matrix_Reloaded::testit',
				table => 'options',
				sort => 'o_sort',
...
#::logDebug("tag_options matrix ary: " . ::uneval($ary));
		...
		for(qw/code description sale_price/) {
			push @rf, ($map->{$_} || $_);
		}
		my $lcol = $map->{sku} || 'sku';

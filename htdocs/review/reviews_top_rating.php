...
//BodyHeader("Welcome to $sitename");
		...
		$sql = "SELECT id, SUBSTRING_INDEX(review, ' ', $show_words) as review, source, review_item_id, username,  date_format(date_added, '%M %d, %Y') as date_added, rating 
		FROM review 
		WHERE approve = 'y' 
		...
		?>
<span class="style2"><a href="<?php echo "$directory"; ?>/review-item/<?php echo "$review_item_id"; ?>.php?<?php echo htmlspecialchars(SID); ?>" class="style4"><?php echo ucfirst($item_name); ?></a>
<?php		  

...
<p align="center" class="style1">Welcome to Five Star Review Script <em>UPGRADE</em> </p>
<hr>
<p class="style2"> The assumption is that you are using a previous version of Five Star Review and wish to upgrade to version 4. </p>
<p class="style2"><strong>Step 1:</strong> Make a complete backup copy of all your review script files and all database tables. This is important! Should something go wrong during the upgrade process, you'll want to have those files handy! It is possible you are running an old version that may cause a problem during the upgrade process. Should that occur, restore your database and files. Then contact <a href="mailto:webmaster@review-script.com">webmaster@review-script.com</a> with the details of what happened and we will check into it. </p>
<p class="style2"><strong>Step 2: </strong> Make note of all the settings in your config.php and functions.php file. You will need this information during the upgrade process. </p>
<p class="style2"><strong>Step 3: </strong>Upload ALL files from the version 4 zip file. The only file you should not replace is body.php. However, you need to open your current body.php and find this at the top:</p>
<p class="style2">&lt;? <br />
  function BodyHeader($title)<br />
  {<br />
  //if a session does not yet exist for this user, start one<br />
  //session_start();<br />
?&gt;</p>
<p class="style2">REPLACE with this:</p>
<p class="style2">&lt;?php <br />
  function BodyHeader($title)<br />
  {<br />
  include (&quot;config.php&quot;);<br />
?&gt; </p>
<p class="style2"><strong>Step 4: </strong><a href="http://www.google.com/search?hl=en&amp;lr=&amp;rls=GGLD%2CGGLD%3A2004-26%2CGGLD%3Aen&amp;q=how+do+i+chmod%3F+ftp" target="_blank">CHMOD</a> the following files to 666: config.php and functions.php. CHMOD the following directories to 777: /review/usercp/upload/images, /review/images/items and /review/admin/images_uploaded. </p>
<p class="style2">This can be done using an ftp program or with telnet, ssh or ssh2.</p>
<p class="style2">After completing Steps 1 - 4, click <a href="filecheck.php" target="_blank">here</a> to check your file permissions. </p>
<p class="style2">&nbsp;</p>
<p class="style2">If you prefer not to mess with all of this you can have this script professionally upgraded for you. The fee is $25. After clicking the Buy Now button, you will need to send an email to <a href="mailto:webmaster@review-script.com">webmaster@review-script.com</a> with the following information: </p>
<p class="style2">1. FTP Address:<br />
  2. FTP Username:<br />
3. FTP Password:<br />
4. URL, Username and Password to access your webhost control panel. This is necessary to set up the mysql database.<br />
5. Access to your current config.php and functions.php files. </p>
<form action="https://www.paypal.com/cgi-bin/webscr" method="post">
  ...
  <input type="image" src="http://www.paypal.com/en_US/i/btn/x-click-but23.gif" border="0" name="submit" alt="Purchase Upgrade Installation Only">
</form>
<p>&nbsp; </p>
<p class="style2">Once Steps 1 - 4 have  been completed, please proceed to the next step.</p>
<form name="form1" method="post" action="upgrade_to_4.php?step=2">
  <div align="right">
...
<form name="form2" method="post" action="upgrade_to_4.php?step=3">
...
//check to see which old version of review script is running and use the appropriate update tables.
if(mysql_num_rows(mysql_query("SHOW TABLES LIKE '".users."'"))==1) { 
run_query_batch($connection, "upgrade_to_4_old.sql");
} else {
 run_query_batch($connection, "upgrade_to_4.sql");
 }
...
?>
<form name="form1" method="post" action="upgrade_to_4.php?step=4">
  <div align="right">
    <input name="step" type="hidden" id="step" value="4">
	    <input name="table_prefix" type="hidden" id="step" value="<?php echo "$table_prefix"; ?>">
...
<form name="form2" method="post" action="upgrade_to_4.php?step=5">
<table width="80%"  border="0" align="center" cellpadding="0" cellspacing="0">
  <tr>
  ...
  <tr>
    ...
    <tr>
    <td class="style2">Can the user upload an image with their review?</td>
    <td><select name="user_upload" id="user_upload">
...
\$user_upload = "$_POST[user_upload]";  //can the user upload an image with their review?  y or n.
...
echo "$msg"; 
  ...
  ?> </p>
	<p class="style2">Your script and database are now successfully upgraded. You now need to log into the admin area and do the following:</p>
	<p class="style2">1. Create a Category<br />
    2.  Create a Review Item<br />
    3.  Go to <a href=<?php echo "$directory"; ?>/demo.php>demo.php</a> and post a test review</p>
	<p class="style2">Log into your <a href="<?php echo "$directory"; ?>/admin/index.php">administration</a> area with the following:</p>

...
$sel_record = clean($_GET['sel_record']);
		...
		$sel_record = clean($_POST['sel_record']);
}
$item_id=0;
		if (!$sel_record) {
header("Location: $directory/admin/admin_edit1.php?".sid);
			exit;
		}
		
			$sql = "SELECT * FROM review WHERE id='$sel_record'";
...
BodyHeader("Edit a Review!","","");
?>
<br />

  <h1 align="center"><font face="Verdana, Arial, Helvetica, sans-serif" size="3">You have selected 
    the following review for Editing:</font></h1><br />
<br />


<FORM method="POST" action="admin_edit3.php?<?php echo htmlspecialchars(SID); ?>">
  <input type="hidden" name="sel_record" value="<?php echo $sel_record ?>">
  <table align="center" cellpadding=5 cellspacing=5>
    <tr> 
      <td valign=top><strong>Summary:</strong></td>
      <td valign=top><input name="summary" type="text" value="<?php echo htmlentities("$summary"); ?>">      </td>
    </tr>
    <tr> 
      <td valign=top><strong>Review:</strong></td>
      <td valign=top>
	  <?php echo "<textarea name=review cols=35 rows=8>" . htmlentities($review) . "</textarea>"; ?>	  </td>
    </tr>
    <tr> 
      <td valign=top><strong>Source:</strong></td>
      <td valign=top><input name="source" type="text" value="<?php echo htmlentities("$source"); ?>"></td>
    </tr>
    <tr> 
      <td valign=top><strong>Date Added:</strong></td>
      <td valign=top><INPUT type="text" name="date_added" value=<?php echo "$date_added" ?> size=35 maxlength=75></td>
    </tr>
  	<tr>
		<td colspan=2>
	...
	<tr> 
      <td align=center colspan=2> <INPUT type="submit" value="Approve Review">      </td>
    </tr>
  </table>

  ...
  <table width="100%"  border="0" align="center" cellpadding="6" cellspacing="0">
    <tr>
      <td><?php
		...
		$res=mysql_query('Select * from review_category_list where parent_id = '.$category_id.' ORDER BY category');
		...
		while ( $row = mysql_fetch_array($result))
			...
			$catQ = $category_id;		
			if ($category_id != -1) {
				echo "<tr><td colspan=2><B>". getCategoryName($category_id) . "</B></td></tr>";
			}
			...
			if ($i % 2) { echo "<tr><td width=50% valign=top><a href='$directory/review-item/$item_id.php?" . htmlspecialchars(SID) . "'>$item_name</a>";
        ...
        <?php } //if average
			...
			if ($average <= "5.0") { echo " ($average_for Stars out of $total reviews.):";  } ?>
 
        <?php   //show stars if there is no review criteria
        ...
        <?php  }
			else { echo "<td width=50%><a href='$directory/review-item/$item_id.php?" . htmlspecialchars(SID) . "'>$item_name</a>";
			?>
        <span class="ad_title"><br />
        <b>Avg. Customer Review</b>
        <?php 
$average_for = sprintf ("%01.1f", $average); 
if ($average != "10.0") { echo " ($average_for Stars out of $total reviews.):";  } 
 $display = ($average/5)*100; ?>
        <div class="rating_bar">
          <div style="width:<?php echo "$display"; ?>%"></div>
        </div><br />

...
{
  mysql_query( "CREATE TABLE {$pre}dept ( id int(11) NOT NULL auto_increment, name varchar(255) NOT NULL default '', options int(11) NOT NULL default '0', PRIMARY KEY  (id)) TYPE=MyISAM;" );
...
{
  mysql_query( "CREATE TABLE {$pre}user ( id int(11) NOT NULL auto_increment, name varchar(70) NOT NULL default '', email varchar(255) NOT NULL default '', sms varchar(255) NOT NULL default '', signature text NOT NULL, password varchar(20) NOT NULL default '', admin tinyint(4) NOT NULL default '0', date int(11) NOT NULL default '0', lastlogin int(11) NOT NULL default '0', notify int(11) NOT NULL default '0',  PRIMARY KEY  (id)) TYPE=MyISAM;" );
}
...
if( !mysql_query( "SELECT COUNT(*) FROM {$pre}options" ) )
{
  ...
  mysql_query( "INSERT INTO {$pre}options ( name, num, text ) VALUES ( 'outsidebackground', '', '#FDD6AA');" );
  mysql_query( "INSERT INTO {$pre}options ( name, num, text ) VALUES ( 'border', '', '#0B6E1D');" );
  mysql_query( "INSERT INTO {$pre}options ( name, num, text ) VALUES ( 'topbar', '', '#FF8A00');" );
  ...
  $data = "Help Desk Notification - New Ticket Reply";
  ...
  $data = "Help Desk Notification - New Ticket Created";
...
/********************************************************** PHP */?>
<div class="title">Setting Up <?= $script_name ?></div><br />
<?/* PHP **********************************************************/
...
if( $_POST[cmd] == "admin" )
{
  ...
  else
  {
    if( !$admin_exists )
    {
      mysql_query( "INSERT INTO {$pre}user ( name, email, password, admin, date ) VALUES ( '{$_POST[name]}', '{$_POST[email]}', '{$_POST[password1]}', '1', '" . time( ) . "' )" );
...
<form action="setup.php" method="post">
  ...
  <tr><td><label for="name">Name: </td><td><input type="text" name="name" size="30" value="<?= field( $_POST[name] ) ?>" /></label></td></tr>
...
<tr><td>
  <div class="graycontainer">
    ...
    you may <a href="<?= $HD_URL_LOGIN ?>">retrieve</a> it.
  </div>
</td></tr>
</table>
<?/* PHP **********************************************************/

...
void toolbox_load(struct LPMAPI* api)
{
   ...
   LMAPI->register_var("newlist-procmail", "no", "Maintenance",
     "When creating a new list, do we need to make procmail recipes?",
     "newlist-procmail = no", VAR_BOOL, VAR_GLOBAL);
   ...
   LMAPI->add_cmdarg("-procmail", 0, NULL, cmdarg_procmail); 

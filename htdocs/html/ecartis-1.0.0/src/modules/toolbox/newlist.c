...
void make_list_aliases(const char *listname, const char *siteconfig, const char *hostname)
{
   char tempbuf[BIG_BUF];

   if (LMAPI->get_bool("newlist-procmail")) {
      printf("\n# Procmail Recipes for '%s' mailing list.\n", listname);

      printf(":0\n* ^To: %s@%s\n|%s/%s %s-s %s\n",
              listname, hostname, LMAPI->get_string("listserver-bin-dir"),
...
void newlist(const char *listname)
{
   ...
   make_list_aliases(listname, siteconfig, hostname);
...
CMDARG_HANDLER(cmdarg_procmail)
{

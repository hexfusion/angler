...
/*
Grouper GPL v1.4.2
Copyright (c) 2003-4 Antone Roundy (Modified by Sam Batschelet)
...
function GrouperBuildQuery($searchterms) {
	...
	if (is_array($groupersources[$grouperconf['source']])) {
		...
		case 'usgs': $query=GrouperBuildQuery2($searchterms); break;
		default: $query=''; break;
		}
	}
	else $query='';
...
function GrouperBuildQuery0($searchterms) {
	...
	return ((substr($srcConf['querystart'],0,1)=='/')?'':'/').
		$srcConf['querystart'].'hl='.$srcConf['language'].
		'&num='.ceil($grouperconf['maxitems']*1.3).
		'&edition='.$srcConf['edition'].'&ie='.$srcConf['encoding'].
		'&sa=G'.$srcConf['sort'.$srcConf['sortby']].'&q='.urlencode($searchterms);
}
...
function GrouperBuildQuery2($searchterms) {
	global $groupersources;
	$srcConf=&$groupersources['usgs'];
	return ((substr($srcConf['querystart'],0,1)=='/')?'':'/').
		$srcConf['querystart'].
